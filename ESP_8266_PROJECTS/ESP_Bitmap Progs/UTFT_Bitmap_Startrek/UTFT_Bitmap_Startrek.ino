...
extern unsigned int infobmap[0x400];
extern unsigned int iconbmap[0x400];
extern unsigned int tuxbmap[0x400];
...
void loop()
{
  ...
  for (int x=0; x<10; x++)
    for (int y=0; y<7; y++)
      myGLCD.drawBitmap (x*32, y*32, 32, 32, infobmap);
  ...
  {
    x+=(s*32);
    myGLCD.drawBitmap (x, 0, 32, 32, tuxbmap, s+1);
  }
  x=0;
  for (int s=4; s>0; s--)
  {
    myGLCD.drawBitmap (x, 224-(s*32), 32, 32, iconbmap, s);
    x+=(s*32);
  }

  delay(5000);
   myGLCD.fillScr(255, 255, 255);
   myGLCD.drawBitmap (105, 80, 126, 126, iconbmap, 1);
    delay(5000);
    }
