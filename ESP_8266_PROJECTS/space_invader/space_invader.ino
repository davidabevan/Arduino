...
//#define TFT_MISO 19 //25 not connected
#define led_pin  D8 //D8
#define TFT_CLK  D5 //D5
#define TFT_MOSI D7 //D7
//#define TFT_DC    D2 //D2
//#define TFT_CS    D4 //D1
#define TFT_RST  RST  //RST
#define TFT_DC D2
#define TFT_CS D1
//#define BUTTON_A_PIN D3//left
//#define BUTTON_B_PIN D4//right
//#define BUTTON_C_PIN D6//fire
...
if (((0<=(x))&&(0>=(x-XLIVEWIDE)))&&(y==7)) {return pgm_read_byte(&LIVE[(x)]);}
return 0x00;}
...
SSD1306.ssd1306_send_byte(background(x,y,space)|LivePrint(x,y,space)|Vesso(x,y,space)|UFOWrite(x,y,space)|Monster(x,y,space)|MyShoot(x,y,space)|MonsterShoot(x,y,space)|MYSHIELD);
...
SpriteType=space->MonsterGrid[space->PositionDansGrilleMonsterY][space->PositionDansGrilleMonsterX];
if (SpriteType<8) {ANIMs=(space->anim*14);}else{ANIMs=0;}
if (SpriteType!=-1) { Murge2=SplitSpriteDecalageY(pgm_read_byte(&Monsters[(WriteMonster14(x-space->MonsterGroupeXpos,space)+SpriteType*14)+ANIMs]),1,space);}else{
Murge2=0x00;
}
return Murge2;    
}else{
...
if (SpriteType<8) {ANIMs=(space->anim*14);}else{ANIMs=0;}
if (SpriteType!=-1) {Murge1=SplitSpriteDecalageY(pgm_read_byte(&Monsters[(WriteMonster14(x-space->MonsterGroupeXpos,space)+SpriteType*14)+ANIMs]),0,space);
}else{Murge1=0x00;}
SpriteType=space->MonsterGrid[space->PositionDansGrilleMonsterY][space->PositionDansGrilleMonsterX];
if (SpriteType<8) {ANIMs=(space->anim*14);}else{ANIMs=0;}
if (SpriteType!=-1) { Murge2=SplitSpriteDecalageY(pgm_read_byte(&Monsters[(WriteMonster14(x-space->MonsterGroupeXpos,space)+SpriteType*14)+ANIMs]),1,space);
}else{Murge2=0x00;}
