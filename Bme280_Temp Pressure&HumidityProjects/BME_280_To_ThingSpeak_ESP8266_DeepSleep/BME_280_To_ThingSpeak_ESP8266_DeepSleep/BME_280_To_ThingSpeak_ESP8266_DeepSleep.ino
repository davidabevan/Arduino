...
//not as good as master not so much serial info displays wifi connections once on startup
...
void setup() {
 ...
 // turnOff(0);
  ...
  turnOff(14);
  turnOff(15);
 Wire.begin(D3,D4);//Needed for Wemos D1  ESP8266 Starts I2C
...
void loop() {
  ...
  //not as good as master not so much serial info 
// // go to deepsleep for 10 minutes
 // system_deep_sleep_set_option(0);
 // system_deep_sleep(10 * 60 * 1000000);// Causes esp RST
 
 // go to deepsleep for 1 minutes
 ESP.deepSleep(60000000, WAKE_RF_DEFAULT); // Sleep for 60 seconds
 //Note Deepsleep causes counter to reset so thingspeak count =0
}
