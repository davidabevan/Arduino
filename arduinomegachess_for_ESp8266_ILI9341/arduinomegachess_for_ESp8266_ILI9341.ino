...
//   M5.Speaker.playMusic(music,5000); // ttttooooodooooooo 
   remains-=200;
  } 
}
//****************************
void beep(int leng) {
//  if (sound) M5.Speaker.setVolume(1); else M5.Speaker.setVolume(0);// toooooooodddddooooooooooooooo            
...
//****************************
void clearstatus() {
  tft.fillRect(240,166,80,20,ILI9341_BLACK);             
  tft.drawFastHLine(10,237,220,ILI9341_BLACK);   
  tft.drawFastHLine(10,239,220,ILI9341_BLACK);  
}  
...
//  prefs.begin("AM_Chess", false); // WILL NOT COMPILE NEED TO INVESTIGATE 
...
//  tft.setBrightness(200);    // BRIGHTNESS = MAX 255// NO CONSTRUCTOR FOR ILI9341
  ...
  tft.setTextColor(getColor(230,230,200));
  tft.setTextSize(2);  
  tft.setCursor(50,30); tft.print("Arduino Mega Chess");
  tft.setCursor(88,50); tft.print("for M5Stack");
  tft.setTextColor(CGRAY);
  tft.setCursor(62,70); tft.print("by Sergey Urusov");
  tft.setTextSize(1);  
  tft.setCursor(100,95); tft.print("urusovsv@gmail.com");
  tft.setCursor(85,110); tft.print("version 1.0, engine 1.4");
  tft.setCursor(80,220); tft.print("press any key to READ this");
  tft.setTextColor(CWHITEFIG);
  tft.drawRect(38,150,60,30,CGRAY);    
  tft.drawRect(130,150,60,30,CGRAY);    
  tft.drawRect(225,150,60,30,CGRAY);    
  tft.setTextSize(1);  
  tft.setCursor(57,162); tft.print("Left");
  tft.setCursor(143,162); tft.print("Choice");
  tft.setCursor(240,162); tft.print("Right");
  tft.setCursor(110,190); tft.print("long press - menu");
  tft.setCursor(110,200); tft.print("or exit from menu");
...
void initboard() {        
  ...
  tft.setTextSize(1);  
  for (int j=1;j<9;j++) {
   tft.setCursor(0,j*28-17);  
   if (rotate) tft.print(j); else tft.print(9-j);
  }
  for (byte i=1;i<9;i++) { 
   tft.setCursor(i*28-9,227);    
...
void loop() {        
  ...
  if (solving) {        
    ...
    if (lastscore>9000) {       
      tft.setTextSize(1);    
      tft.setTextColor(CGREEN,CBLACK);       
      tft.setCursor(260,176); 
      if (lastscore<9999) {
       tft.print("# in "); 
...
//   M5.update(); 
//   if (M5.BtnB.wasPressed()) // toooodoooo
    if (lastpressed==0) lastpressed=millis();    // 
//   if (M5.BtnA.wasPressed()) //todddddddddddddddoooooooooooooooooo
    if (lastpressedleft==0) lastpressedleft=millis();    // 
//   if (M5.BtnC.wasPressed()) //todoooooooooooo
   ...
   if (!menu&&!isstatus&&rotate==cur_step%2) { // AI
    ...
    tft.drawLine(308,175,308+9*sin(angle),175+9*cos(angle),CGRAY);   
    delay(200);    
   }
   ...
   if (!menu&&!isstatus&&rotate!=cur_step%2) {  //  
    ...
    //if (M5.BtnB.isReleased()&&lastpressed) 
      if (millis()-lastpressed>500) { //   todoooooooo  
         if (!isstatus) {
...
//         short cur_step_=prefs.getShort("cur_step");// PREFS AGAIN TO CHECKOUT
//         short limit_=prefs.getShort("limit");
         if (cur_step>0&&cur_step<1000&&limit>=0&&limit<3) {
          initboard();
//          cur_step=cur_step_; limit=limit_;//NNNNNNNNNNNN
...
//         prefs.putShort("cur_step",cur_step);// TO CHECKOUT
...
void animategreen_step(int nstep, boolean hide) {     
 ...
 if (lastanimatedgreen.x1==steps[nstep].x1&&lastanimatedgreen.y1==steps[nstep].y1&&
  ...
  if (!rotate) tft.drawRect(i2*28+8,j2*28+1,27,27,CGREEN);    
   else tft.drawRect((7-i2)*28+8,(7-j2)*28+1,27,27,CGREEN);          
  return;
 }
 ...
 while (j!=steps[nstep].x2||i!=steps[nstep].y2) {  
  show_fig(i,j); 
  if (!hide) 
   if (!rotate) tft.drawRect(i*28+8,j*28+1,27,27,CGREEN);    
    else tft.drawRect((7-i)*28+8,(7-j)*28+1,27,27,CGREEN);    
  int mj=0;  
...
uint16_t color,color_cont;  
   ...
   tft.fillRect(ii*28+7,jj*28,29,29,color);    
...
uint16_t color=getColor(220,220,200);   
 ...
 while (j!=steps[nstep].x2||i!=steps[nstep].y2) {  
  show_fig(i,j); 
  if (!hide) 
   if (!rotate) tft.drawRect(i*28+8,j*28+1,27,27,color);    
    else tft.drawRect((7-i)*28+8,(7-j)*28+1,27,27,color);    
  int mj=0;  
  if (j!=steps[nstep].x2) { j+=dj; mj=1; }   
  if (abs(steps[nstep].fig1)==fn&&mj==1) continue;
  if (i!=steps[nstep].y2) i+=di;  
...
void show_status() {               
   ...
   tft.drawFastHLine(10,237,cur,CGRAY);                  
   tft.drawFastHLine(10,239,progress*2,CGRAY);    
   int m=tim/60;
   int s=tim%60;
   tft.setTextWrap(1);
   tft.setTextColor(CGRAY);
   tft.setTextSize(1);
   tft.fillRect(240,166,80,10,CBLACK);                
   tft.setCursor(242,166); 
   if (m>0) { tft.print(m); tft.print(":"); }
   if (s>0) {
    if (s<10&&m>0) tft.print("0"); 
    tft.print(s);
   } else if (m>0) tft.print("00"); else tft.print("0");   
   tft.setCursor(280,166);   
   tft.print(cur_level);   
   tft.setCursor(242,176); 
   if (cur_step%2==1) tft.setTextColor(CWHITE); else tft.setTextColor(CGRAY);   
   if (lastbest.fig1!=steps[0].fig1||lastbest.x1!=steps[0].x1||lastbest.y1!=steps[0].y1||
       lastbest.x2!=steps[0].x2||lastbest.y2!=steps[0].y2) {     
     lastbest=steps[0];         
     tft.fillRect(240,176,80,10,CBLACK);                  
     tft.setCursor(242,176);
     tft.setTextColor(CGRAY);     
...
uint16_t color,color_cont;  
   ...
   if (rotate) { jj=7-j; ii=7-i; }   
   tft.fillRect(ii*28+7,jj*28,29,29,color);    
   tft.drawRect(ii*28+7,jj*28,29,29,CGRAY);  
...
uint16_t color=getColor(200,200,180);  
  tft.fillRect(234,0,90,160,getColor(8,8,8));  
  tft.fillRect(234,0,90,24,CDARK);  
  tft.setTextSize(1); 
  tft.setTextColor(getColor(160,160,140),CDARK);    
  tft.setCursor(241,2);    
  tft.print(" ArduinoMega"); 
  tft.setCursor(240,14);    
  tft.print("Chess M5Stack");   
  int i=1; int y=27;  
...
uint16_t color=getColor(220,220,200);
  ...
  tft.fillRect(233,190,86,50,CBLACK);    
  tft.setTextSize(1);   
  if (menu==1) tft.setTextColor(CBLACK,CBLUE); else tft.setTextColor(color,CGRAY2);   
  if (submenu) {
    tft.setCursor(242,190); tft.print(" New Game "); 
  } else {  
   tft.setCursor(233,190); tft.print(" Back "); 
  } 
  if (menu==2) tft.setTextColor(CBLACK,CBLUE); else tft.setTextColor(color,CGRAY2);   
  if (submenu) {
    tft.setCursor(242,203); tft.print(" Load Game "); 
  } else {    
   tft.setCursor(271,190); tft.print(" Rotate "); 
  } 
  if (menu==3) tft.setTextColor(CBLACK,CBLUE); else tft.setTextColor(color,CGRAY2);   
  if (submenu) {
    tft.setCursor(242,216); tft.print(" Save Game ");   
  } else {    
   tft.setCursor(242,203); tft.print(" Game menu ");   
  } 
  if (menu==4) tft.setTextColor(CBLACK,CBLUE); else tft.setTextColor(color,CGRAY2);   
  if (submenu) {
    tft.setCursor(242,229); tft.print(" USB FEN "); 
  } else {    
   tft.setCursor(242,216); tft.print(" Level "); 
   switch (limit) {
   case 0: tft.print("LOW "); break;
   case 1: tft.print("MED "); break;
   case 2: tft.print("HIGH"); break;
   }   
  }   
  if (menu==5) tft.setTextColor(CBLACK,CBLUE); else tft.setTextColor(color,CGRAY2);   
  if (submenu) {
  } else {    
   tft.setCursor(242,227); tft.print(" Sound ");  
   if (sound) tft.print("ON  "); else tft.print("OFF ");
  } 
}
...
boolean check_on; 
 if (!hidden) {
  tft.fillRect(240,166,80,20,CBLACK);             
  tft.drawFastHLine(10,237,220,CDARK);   
 }   
 starttime=millis();    
 isstatus=1; 
 ...
 if (cur_var==0||is_drawn()) {  
  beep(500);
  tft.drawFastHLine(10,237,220,CBLACK);   
  tft.drawFastHLine(10,239,220,CBLACK);   
  isstatus=0;
  solving=false;  
 ...
 while (l<6) {
  cur_level=l+1;  
  progress=0;
  tft.drawFastHLine(10,239,220,CDARK);  
  MINDEPTH=LMIN[l];  
  MAXDEPTH=LMAX[l];            
  if (!hidden) {
