...
//********Setup Bitmap Array*********
extern unsigned int Arduino[];//Splash screen.****Had to remove memory footprint as freezing during wifi init
...
extern unsigned int scatteredcloud[];
extern unsigned int mostlycloudy[];
extern unsigned int PartlyCloudy[];// Todo
extern unsigned int scatteredclouds[];//Todo
extern unsigned int fog[];// Todo
...
void setup() {
  ...
  tft.setTextColor(ILI9341_YELLOW); tft.setTextSize(2);
  //tft.println(1234.56);
  // WiFi.begin();//(WIFI_SSID, WIFI_PWD);//**********TRying WiFI persistence.
  //int counter = 0; //Needed wont connect without db 26/08/2016 Wrong
  //while (WiFi.status() != WL_CONNECTED) {
  //delay(500);
  //tft.print(".");
  //tft.fillScreen(ILI9341_BLACK);
  //tft.setTextColor(ILI9341_YELLOW); tft.setTextSize(2);
  //tft.println("Connecting to WiFi");
  //     tft.println(WIFI_SSID);//Stops here***************Restart Router
  // counter++; // See line 122
...
if (Counter == 1) {     //Thingspeak update timer. See line 189 now a function
 ...
 // delay (3000);
  ...
  //String Windspeed = getWindSpeed();// not working...Correct library.?
  tft.setCursor(20, 10);
  ...
  tft.print("WindSpeed");
  //  tft.print(Windspeed);// Not working
  delay (5000);
  //To Do wind strength and direction
  //To Do Time at the bottome again?????
  //To Do Timer for updating from Wunderground every 10 mins
  //To Do arduino bmp splash screen at startup
  ...
  if (weathertext == "Clear") {
    ...
    tft.drawBitmap (30, 50, 42, 42, chancerain);
  }
  else if (weathertext == "Overcast") {
    tft.drawBitmap (30, 50, 42, 42, overcast);
  }
  else if (weathertext == "Cloudy") {
    tft.drawBitmap (30, 50, 42, 42, overcast);
  }
  else if (weathertext == "Rain") {
    tft.drawBitmap (30, 50, 42, 42, Rain);
  }

  else if (weathertext == "Mostly Cloudy") {
    tft.drawBitmap (30, 50, 42, 42, mostlycloudy);
  }
   else if (weathertext == "Partly Cloudy") {
    tft.drawBitmap (30, 50, 42, 42, PartlyCloudy);
  }
  delay(3000);
}
...
void GetWeatherUpdate() {
  ...
  Serial.print( "Updating conditions...");
  wunderground.updateConditions(WUNDERGRROUND_API_KEY, WUNDERGRROUND_LANGUAGE, WUNDERGROUND_COUNTRY, WUNDERGROUND_CITY);
  Serial.print ("Updating forecasts...");
...
//*******************From wunderground metric try to get today forecast icons working
//************************First part*************************
